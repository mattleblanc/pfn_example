...
def smart_animate(i):
    ...
    # # frame number starting from 1, is multiplied by the phase which is nframes / nstages
    # # is there any way to pre-fill this array so it isn't created/calculated every time a frame is drawn?
    # phase = []

    # # each event is a stage; in the future when animating clustering algo, each step in clustering history is a stage
    # for j in range(0, nstages - 1):
    #     # multiply nframes/nstages by j+1
    #     phase[j] = j * stage_size
    #     # this will set the frame-number (i) cutoffs for each phase
